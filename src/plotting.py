...
class Plotter:
    ...
    COLORS = {
        ...
        "good": "#059669",    # green for ideal annotation
    }
    ...
    def _apply_theme(self, ax):
        ...
        ax.grid(True, color=self.COLORS["grid"], linewidth=0.8, zorder=0)
    ...
    def _new_fig(self):
        """Create a new figure using the shared visual theme."""
        fig, ax = plt.subplots(figsize=(9, 6))
        fig.patch.set_facecolor("white")
        self._apply_theme(ax)
        return fig, ax

    def _ols_hline(self, ax, value, label):
        """Draw a horizontal OLS baseline dashed line."""
        if value is not None:
            ax.axhline(
                value,
                linestyle="--",
                color=self.COLORS["ols"],
                linewidth=1.4,
                alpha=0.85,
                label=label,
                zorder=3
    ...
    def plot_error_curves(self, results, dataset_name):
        ...
        if ridge_rows:
            ...
            ax.plot(
                lambdas,
                train_mse,
                marker=self.RIDGE_MARKER,
                color=self.COLORS["ridge"],
                linestyle="--",
                linewidth=1.6,
                alpha=0.7,
                label="Ridge Train MSE",
                zorder=4
            )
            ax.plot(
                lambdas,
        ...
        if lasso_rows:
            ...
            ax.plot(
                lambdas,
                test_mse,
                marker=self.LASSO_MARKER,
                color=self.COLORS["lasso"],
                linewidth=2.0,
                label="Lasso Test MSE",
                zorder=4
            )

        if ols_row is not None:
            self._ols_hline(ax, ols_row["train_mse"], "OLS Train MSE")
            self._ols_hline(ax, ols_row["test_mse"], "OLS Test MSE")

        ax.set_xscale("log")
        ax.set_xlabel("Regularization strength (λ)", fontsize=12)
        ax.set_ylabel("Mean Squared Error", fontsize=12)
        ax.legend(fontsize=9, framealpha=0.9, edgecolor=self.COLORS["grid"])
    ...
    def plot_prediction_stability(self, results, dataset_name):
        ...
        ridge_rows = self._get_rows(results, "Ridge")
        lasso_rows = [
            r for r in self._get_rows(results, "Lasso")
        ...
        if lasso_rows:
            ...
            ax.plot(
                ...
                color=self.COLORS["lasso"],
                linewidth=2.0,
                label="Lasso prediction change",
                zorder=4
            )

        if ols_row is not None:
            self._ols_hline(ax, ols_row["prediction_change"], "OLS prediction change")
    ...
    def plot_loss_stability(self, results, dataset_name):
        ...
        if lasso_rows:
            ...
            ax.plot(
                ...
                color=self.COLORS["lasso"],
                linewidth=2.0,
                label="Lasso loss change",
                zorder=4
            )

        if ols_row is not None:
            self._ols_hline(ax, ols_row["loss_change"], "OLS loss change")
    ...
    def plot_stability_vs_test_error(self, results, dataset_name):
        ...
        ridge_rows = [
            r for r in self._get_rows(results, "Ridge")
            if r["prediction_change"] is not None
        ]
        lasso_rows = [
            r for r in self._get_rows(results, "Lasso")
        ...
        if ridge_rows:
            ...
            log_lam = np.log10([max(lam, 1e-16) for lam in lambdas])
            ...
            label_lams = {1e-15, 0.001, 1, 5, 30, 100}
            for x, y, lam in zip(xs, ys, lambdas):
                if lam not in label_lams:
                    continue
        ...
        legend_elements = [
            Line2D(
                ...
                marker=self.RIDGE_MARKER,
                color="w",
                markerfacecolor=plt.cm.plasma_r(0.1),
                markersize=9,
                label="Ridge (low λ)"
            ),
            Line2D(
                ...
                marker=self.RIDGE_MARKER,
                color="w",
                markerfacecolor=plt.cm.plasma_r(0.9),
                markersize=9,
                label="Ridge (high λ)"
            ),
            Line2D(
                ...
                marker=self.OLS_MARKER,
                color="w",
                markerfacecolor=self.COLORS["ols"],
                markersize=9,
                label="OLS"
            ),
            Line2D(
        ...
        if lasso_rows:
            legend_elements.insert(
                ...
                Line2D(
                    ...
                    marker=self.LASSO_MARKER,
                    color="w",
                    markerfacecolor=self.COLORS["lasso"],
                    markersize=9,
                    label="Lasso models"
                )
            )
    ...
    def plot_model_complexity(self, results, dataset_name):
        ...
        if lasso_rows:
            ...
            ax.plot(
                ...
                color=self.COLORS["lasso"],
                linewidth=2.0,
                label="Lasso coefficient norm",
                zorder=4
            )

        if ols_row is not None:
            self._ols_hline(ax, ols_row["coef_l2_norm"], "OLS coefficient norm")
