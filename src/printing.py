class ResultPrinter:
    """
    Class responsible for printing experiment result tables.
    """
    ...
    def print_results_table(self, results):
        ...
        for row in results:
            print(
                ...
                f"{self._format_float_width(row['prediction_change'], 14, 6)} "
                f"{self._format_float_width(row['loss_change'], 14, 6)}"
            )
        ...
        for model_name in ["Ridge", "Lasso"]:
            ...
            if len(model_rows) > 0:
                ...
                rows_with_stability = [
                    ...
                    if row["prediction_change"] is not None
                ]

                print(f"\nBest {model_name} by test MSE:")
                print(
                    f"lambda = {best_test['lambda']}, "
                    f"test MSE = {best_test['test_mse']:.4f}"
                )

                if rows_with_stability:
                    best_stability = min(
                        rows_with_stability,
                        key=lambda row: row["prediction_change"]
                    )
    ...
    def print_size_results_table(self, results):
        header = (
